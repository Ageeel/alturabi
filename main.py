...
def main(page: ft.Page):
    ...
    # تأكد من مطابقة حالة الأحرف مع المجلدات الفعلية داخل assets[span_1](start_span)[span_1](end_span)
    page.fonts = {
        "font": "font/ar.ttf",
    ...
    # --- ميزة النسخ الاحتياطي والاستعادة عبر السحب ---
    def save_backup_result(e: ft.FilePickerResultEvent):
        if e.path:
            try:
                shutil.copy("data.db", e.path)
                page.snack_bar = ft.SnackBar(ft.Text("تم تصدير النسخة الاحتياطية بنجاح! 💾", text_align=ft.TextAlign.RIGHT), bgcolor=ft.colors.GREEN_400)
                page.snack_bar.open = True
            except Exception as ex:
                page.snack_bar = ft.SnackBar(ft.Text(f"فشل التصدير: {str(ex)}", text_align=ft.TextAlign.RIGHT), bgcolor=ft.colors.RED_400)
                page.snack_bar.open = True
            page.update()
    ...
    def restore_backup_result(e: ft.FilePickerResultEvent):
        if e.files:
            ...
            try:
                ...
                page.snack_bar = ft.SnackBar(ft.Text("تم استيراد قاعدة البيانات بنجاح! جاري التحديث... 🔄", text_align=ft.TextAlign.RIGHT), bgcolor=ft.colors.GREEN_400)
                page.snack_bar.open = True
                refresh_ui_data()
            except Exception as ex:
                page.snack_bar = ft.SnackBar(ft.Text(f"فشل الاستيراد: {str(ex)}", text_align=ft.TextAlign.RIGHT), bgcolor=ft.colors.RED_400)
                page.snack_bar.open = True
            page.update()
    ...
    def on_header_pan_update(e: ft.DragUpdateEvent):
        if e.delta_x > 15:  
            file_picker_open.pick_files(allowed_extensions=["db"], dialog_title="اختر ملف قاعدة البيانات لاستيرادها")
        elif e.delta_x < -15:  
            file_picker_save.save_file(file_name=f"backup_{datetime.now().strftime('%Y%m%d')}.db", dialog_title="اختر مكان تصدير النسخة الاحتياطية")

    # --- إدارة نافذة تأكيد الحذف ---
    def close_dlg(e):
        confirm_dialog.open = False # تعديل آمن للتوافق مع ديفايس أندرويد[span_2](start_span)[span_2](end_span)
        page.update()
    ...
    def delete_confirmed(debt_id):
        ...
        confirm_dialog.open = False # تعديل آمن للتوافق مع ديفايس أندرويد[span_3](start_span)[span_3](end_span)
        active_debt_id = None
        refresh_ui_data()
    ...
    def show_confirm_dialog(debt_id):
        ...
        page.dialog = confirm_dialog # تعديل طريقة الفتح البرمجية المتوافقة[span_4](start_span)[span_4](end_span)
    ...
    def save_new_debt(e):
        ...
        add_dialog.open = False # إغلاق فوري متوافق وآمن[span_5](start_span)[span_5](end_span)
    ...
    def open_add_dialog_fast(edit_id=None, name="", amount=0, dtype="madin", due_date=None):
        ...
        page.dialog = add_dialog # تعديل طريقة الفتح البرمجية المتوافقة[span_6](start_span)[span_6](end_span)
    ...
    def build_debt_card(debt_id, name, amount, currency, db_days_passed, db_days_left, debt_type, record_date, due_date):
        ...
        if due_date:
            try:
                ...
                d_pure = date(d_parts[0], d_parts[1], d_pure)
        ...
        if is_completed:
            sub_info_control = ft.Row(
                controls=[
                    ft.Icon(name=ft.icons.CHECK_CIRCLE_ROUNDED, color="#4caf50", size=15),
                    ft.Text("تم السداد بالكامل", size=12, color="#4caf50", text_align=ft.TextAlign.RIGHT)
                ],
                spacing=4,
                alignment=ft.MainAxisAlignment.START
            )
        else:
            sub_info_control = ft.Text(
                f"مضى {days_passed} يوم ومتبقي {days_left} يوم للسداد", 
                size=12, 
                color="#757575", 
                ...
                controls=[
                    ft.Column(
                        controls=[
                            ...
                            ft.Text(f"{formatted_amount} {currency}", size=15, color="#d4af37" if not is_completed else "#9e9e9e", weight=ft.FontWeight.W_600, text_align=ft.TextAlign.RIGHT),
                            sub_info_control,
                        ], horizontal_alignment=ft.CrossAxisAlignment.START, spacing=4, expand=True
                    ),
    ...
    empty_view = ft.Container(
        content=ft.Column(
            controls=[
                ft.Container(content=ft.Icon(name=ft.icons.CHECK, color=ft.colors.WHITE, size=20), alignment=ft.alignment.center, width=30, height=30, bgcolor="#bdbdbd", shape=ft.BoxShape.CIRCLE),
                ft.Container(height=1), ft.Text("قائمتك فارغة", color="#bdbdbd", size=13, weight=ft.FontWeight.W_200)
    ...
    def refresh_ui_data():
        ...
        icon_madin = ft.Badge(
            content=ft.Icon(ft.icons.ASSIGNMENT_IND_ROUNDED), 
            text=str(count_madin), 
            bgcolor="#ec407a", 
            text_color=ft.colors.WHITE,
            alignment=badge_alignment,
            offset=badge_offset
        ) if count_madin > 0 else ft.Icon(ft.icons.ASSIGNMENT_IND_ROUNDED)

        icon_done = ft.Badge(
            content=ft.Icon(ft.icons.CHECK_CIRCLE_ROUNDED), 
            text=str(count_total_done), 
            bgcolor="#4caf50", 
            text_color=ft.colors.WHITE,
            alignment=badge_alignment,
            offset=badge_offset
        ) if count_total_done > 0 else ft.Icon(ft.icons.CHECK_CIRCLE_ROUNDED)

        icon_adin = ft.Badge(
            content=ft.Icon(ft.icons.SUPERVISOR_ACCOUNT_ROUNDED), 
            text=str(count_adin), 
            bgcolor="#ec407a", 
            text_color=ft.colors.WHITE,
            alignment=badge_alignment,
            offset=badge_offset
        ) if count_adin > 0 else ft.Icon(ft.icons.SUPERVISOR_ACCOUNT_ROUNDED)
        ...
        page.navigation_bar = ft.NavigationBar(
            ...
            destinations=[
                ft.NavigationDestination(
                    icon_content=icon_madin,  
                    label="مدين منهم"
                ),
                ft.NavigationDestination(
                    icon_content=icon_done,
                    label="مكتمل"
                ),
                ft.NavigationDestination(
                    icon_content=icon_adin,
                    label="أدين لهم"
                ),
            ]
        )
        ...
        content=ft.Container(
            content=ft.Column(
                controls=[
                    ft.Row(
                        controls=[
                            ft.Column([ft.Text("رصيد المدينين", color=ft.colors.WHITE70, size=12), total_madin_text], horizontal_alignment=ft.CrossAxisAlignment.CENTER, expand=True),
                            ft.Column([ft.Text("رصيد الدائنين", color=ft.colors.WHITE70, size=12), total_adin_text], horizontal_alignment=ft.CrossAxisAlignment.CENTER, expand=True)
                        ], alignment=ft.MainAxisAlignment.SPACE_AROUND
                    ),
                ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=5
            ), bgcolor="#ec407a", padding=ft.padding.only(top=30, bottom=20, left=10, right=10),
        )
    )
...
ft.app(target=main, assets_dir="assets", upload_dir="downloads")
