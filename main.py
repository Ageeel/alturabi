...
def main(page: ft.Page):
    ...
    def start_add_operation(e):
        ...
        operation_mode = "add"
        amount_input.value = ""
        # في نسخة 0.19.0 نقوم بتعديل الـ hint_text بدلاً من label لضمان اتجاهه لليمين
        amount_input.hint_text = f"المبلغ المراد إضافته لـ {int(stored_amount)}"
    ...
    def start_subtract_operation(e):
        ...
        amount_input.value = ""
        amount_input.hint_text = f"المبلغ المراد طرحه من {int(stored_amount)}"
    ...
    # الحل لنسخة 0.19.0: تم استبدال label بـ hint_text لأن الـ hint يتبع محاذاة اليمين تماماً
