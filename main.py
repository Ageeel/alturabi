...
def main(page: ft.Page):
    ...
    # بناء واجهة الإدخال كـ Container داخلي يأخذ الشاشة كاملة بخلفية معتمة لتغطية القائمة
    input_layer = ft.Container(
        content=ft.Container(
            content=ft.Column(
                controls=[
                    ft.Row([ft.Text("إضافة / تعديل سجل", weight=ft.FontWeight.BOLD, size=16), ft.IconButton(ft.icons.CLOSE, on_click=close_input_container)], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                    ft.Divider(),
            ...
            ), padding=20, bgcolor=ft.colors.WHITE, border_radius=15, width=280
        ),
        alignment=ft.alignment.center,
        bgcolor="rgba(0,0,0,0.4)", # معتم شفاف لحجب القائمة بالخلف
        expand=True,
    ...
    def build_main_ui():
        ...
        # استخدام Stack ليتم تركيب طبقة الحاوية فوق عناصر القائمة الرئيسية عند إظهارها
        page.add(
            ft.Stack(
                controls=[
