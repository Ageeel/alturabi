...
def init_db():
    ...
    """) # cite: main.py
    # جدول الإعدادات لحفظ حالة الـ Onboarding
    cursor.execute("""
    ...
    """)
    # افتراضياً، لم يتم رؤية شاشة Onboarding بعد
...
def main(page: ft.Page):
    ...
    # --- واجهة الـ Onboarding التتابعية المتوافقة مع 0.19.0 ---
    current_step = 0
    
    steps_data = [
        {"icon": ft.icons.ACCOUNT_BALANCE_WALLET_ROUNDED, "title": "إدارة ديونك بسهولة", "desc": "تطبيقك المثالي لتسجيل الديون ومتابعة المبالغ المستحقة لك أو عليك بدقة وبأمان."},
    ...
    dots_row = ft.Row([
        ft.Container(width=10, height=10, bgcolor="#ec407a", border_radius=5),
    ...
    def next_step(e):
        ...
        if current_step < 2:
            ...
            if current_step == 2:
                btn_next.text = "ابدأ الآن"
            page.update()
        else:
            start_app_after_onboarding(None)
    ...
    btn_next = ft.ElevatedButton("التالي", bgcolor="#ec407a", color=ft.colors.WHITE, width=150, height=45, 
                                 style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=10)),
                                 on_click=next_step)
    
    # هنا تم نقل خاصية اللون إلى الـ ButtonStyle لحل مشكلة التوافق تماماً
