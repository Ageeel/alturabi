...
def main(page: ft.Page):
    ...
    # تعريف الـ FAB مسبقاً لاستخدامه داخل الدوال
    ...
    def close_input_container(e):
        input_layer.visible = False
        page.floating_action_button = fab # إعادة إظهار الزر عند الإغلاق
        page.update()
    ...
    def delete_confirmed(debt_id):
        ...
        page.snack_bar = ft.SnackBar(ft.Text("تم حذف السجل بنجاح", text_align=ft.TextAlign.CENTER), bgcolor="#ec407a")
    ...
    def save_new_debt(e):
        ...
        input_layer.visible = False
        page.floating_action_button = fab # إعادة إظهار الزر بعد الحفظ بنجاح
        page.update()
    ...
    btn_quick_add = ft.OutlinedButton(text="إضافة ", style=ft.ButtonStyle(color="#607d8b", bgcolor=ft.colors.WHITE, shape=ft.RoundedRectangleBorder(radius=8), side=ft.BorderSide(color="#e0e0e0", width=1)), width=110, on_click=start_add_operation, visible=False)
    btn_quick_sub = ft.OutlinedButton(text="طرح ", style=ft.ButtonStyle(color="#ec497a", bgcolor=ft.colors.WHITE, shape=ft.RoundedRectangleBorder(radius=8), side=ft.BorderSide(color="#e0e0e0", width=1)), width=110, on_click=start_subtract_operation, visible=False)
    ...
    # عنصر نصي ديناميكي لعنوان الحاوية
    form_title = ft.Text("إضافة سجل جديد", weight=ft.FontWeight.BOLD, size=16)

    input_layer = ft.Container(
        content=ft.Container(
            content=ft.Column(
                controls=[
                    ft.Row([form_title, ft.IconButton(ft.icons.CLOSE, on_click=close_input_container)], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                    ft.Divider(),
                    ft.Row(controls=[btn_madin, btn_adin], alignment=ft.MainAxisAlignment.CENTER, spacing=10, width=240),
                    ft.Container(height=4), name_input, amount_input,
                    ft.Row(controls=[btn_quick_add, btn_quick_sub], alignment=ft.MainAxisAlignment.CENTER, spacing=10, width=240),
                    btn_date_picker, ft.Container(height=10), btn_save
    ...
    def open_add_dialog_fast(edit_id=None, name="", amount=0, dtype="madin", due_date=None):
        ...
        if edit_id:
            form_title.value = "تعديل السجل" # تغيير العنوان عند التعديل
            ...
            form_title.value = "إضافة سجل جديد" # تغيير العنوان عند الإضافة الجديدة
            active_debt_id = None
            name_input.value = ""
            amount_input.value = ""
            btn_save.text = "حفظ"
        ...
        page.floating_action_button = None # إخفاء الزر عند فتح الحاوية
        page.update()
        ...
        return ft.Container(
            content=ft.Row(
                controls=[
                    ...
                    ft.IconButton(icon=ft.icons.DELETE_ROUNDED, icon_color=ft.colors.PINK_200, on_click=lambda e: delete_confirmed(debt_id)),
    ...
    header = ft.Container(content=ft.Column(controls=[ft.Row(controls=[ft.Column([ft.Text("رصيد المدينين", color=ft.colors.WHITE70, size=15), total_madin_text], horizontal_alignment=ft.CrossAxisAlignment.CENTER, expand=True), ft.Column([ft.Text("رصيد الدائنين", color=ft.colors.WHITE70, size=15), total_adin_text], horizontal_alignment=ft.CrossAxisAlignment.CENTER, expand=True)], alignment=ft.MainAxisAlignment.SPACE_AROUND)], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=5), bgcolor="#ec407a", padding=ft.padding.only(top=30, bottom=20, left=10, right=10), margin=ft.margin.only(top=20))
    ...
    onboarding_layer = ft.Container(
        ...
        ]), expand=True, bgcolor=ft.colors.WHITE, visible=False
    )

    main_layout = ft.Stack(
        controls=[
            ft.Column(controls=[header, content_container], expand=True, spacing=0),
            input_layer
        ],
        expand=True,
        visible=False
    ...
    splash_layer = ft.Container(
        content=ft.Column([
            ...
            ft.Text("جار التحميل", size=15, color="#ec407a"),
            ft.Container(height=10),
            ft.ProgressRing(color="#ec407a", width=30, height=30)
    ...
    def handle_splash_timeout(e):
        ...
        if check_onboarding_status():
            ...
            onboarding_layer.visible = True
            page.update()

    timer_trigger = ft.Container(visible=False)
